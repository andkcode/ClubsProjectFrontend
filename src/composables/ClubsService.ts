
import axios from "axios";

const API_URL = "https://localhost:8080/api/clubs";
const AUTH_HEADER = {
    auth: {
        username: "AndrewQuifinity",
        password: "jdDDmvReH-Z$v!tNvFv{j=#49r7I",
    },
    header: {
        'Content-Type': 'application/json',
    }
}

export default class ClubsService {
    static async getAllClubs() {
        try {
            const response = await axios.get(API_URL);
            return response.data;
        } catch (error) {
            console.error(error);
            throw new Error("Failed to fetch clubs");
        }
    }

static async getClubById(id: number) {
    try {
        const response = await axios.get(`${API_URL}/${id}`);
        return response.data;
    } catch (error) {
        console.error(`Error fethcing club: ${id}:`, error);
        throw error;
    }
}

static async createBicycle(club: any) {
    try {
        const response = await axios.post(`${API_URL}/admin`, club, AUTH_HEADER);
        return response.data;
    } catch (error) {
        console.error('Error creating club');
        throw error;
    }
}

static async updateBicycle(id: number, club: any) {
    try {
        const response = await axios.put(`${API_URL}/admin/${id}`, club, AUTH_HEADER);
        return response.data;
    } catch(error) {
        console.error(`Error updating club: ${id}`, error);
        throw error;
    }
}

static async deleteClub(id: number) {
    try {
        await axios.delete(`${API_URL}/admin/${id}`, AUTH_HEADER);
    } catch(error) {
        console.error(`Error deleting club: ${id}`, error);
        throw error;
    }
}
}
